...
async def get_sentiment_window(
    minutes: int = 60,
) -> dict:
    now = datetime.now(timezone.utc)
    window_start = now - timedelta(minutes=minutes)
    half_window = now - timedelta(minutes=minutes // 2)

    async with AsyncSessionLocal() as session:
        recent_stmt = (
            select(func.avg(Post.sentiment), func.count(Post.id))
            .where(Post.analyzed_at.is_not(None))
            .where(Post.created_at >= half_window)
        )
...
async def get_keyword_frequencies(minutes: int = 60) -> dict:
    now = datetime.now(timezone.utc)
    window_start = now - timedelta(minutes=minutes)
    half_window = now - timedelta(minutes=minutes // 2)
...
async def detect_alerts() -> list[dict]:
    ...
    for tag, recent_count in keyword_data["recent"].items():
        ...
        if earlier_count > 0 and recent_count >= earlier_count * settings.spike_multiplier:
            if recent_count >= 3:
                representative = await get_representative_posts([tag])
                alert = {
