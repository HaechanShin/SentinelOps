...
async def test_analyze_sentiment_positive(mock_anthropic):
    mock_anthropic.messages.create.return_value.content = [
        MagicMock(text='{"sentiment": 0.8, "issue_tags": ["praise", "update"]}')
    ]
    ...
    assert "praise" in result["issue_tags"]


@pytest.mark.asyncio
async def test_analyze_sentiment_negative(mock_anthropic):
    mock_anthropic.messages.create.return_value.content = [
        MagicMock(text='{"sentiment": -0.7, "issue_tags": ["server", "bug"]}')
    ]
    ...
    assert "server" in result["issue_tags"]
...
async def test_analyze_sentiment_handles_markdown(mock_anthropic):
    mock_anthropic.messages.create.return_value.content = [
        MagicMock(text='```json\n{"sentiment": 0.3, "issue_tags": ["question"]}\n```')
    ]
    ...
    assert "question" in result["issue_tags"]
